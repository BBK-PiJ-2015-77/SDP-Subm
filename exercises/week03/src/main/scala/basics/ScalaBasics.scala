...
object ScalaBasics {
   ...
   */
  def inRange(start: Int, end: Int): Range = {Range(start,end+1)}
   ...
   */
  def oddRange(n: Int): Range = {Range(1,n*2,2)}
  ...
  def minWhile(r: Array[Int]): Int = {
    if (r.isEmpty) throw new java.lang.UnsupportedOperationException("array is empty")
    val rBuffer = r.toBuffer
    var min = rBuffer(0)
    while (rBuffer.length > 0) {
      if (rBuffer(0) < min) {min = rBuffer(0)}
      rBuffer.remove(0)
    }
    min
  }
  ...
  def minRecursive(r: Array[Int]): Int = {
    if(r.length > 1) {
      if(r(0) < r(1)) {
        if(r.length == 2) {r(0)}
        minRecursive(r.updated(0,r(1)).updated(1,r(0)))
      } else {
        minRecursive(r.takeRight(r.length - 1))
      }
    }
    r(0) // this is what ultimately gets returned
  }
    //needs to be recursive. not sure how to do this without a while statement
  ...
  def splitInHalf(s: String): (String, String) = {
    (s.take(Math.round(s.length/2)), s.takeRight(Math.round(s.length - s.length/2)))
  }
  ...
  def isPalindrome(s: String): Boolean = {
    //use a for loop
    //if s.contains('.','?',',',';','!','-',''')
    //s.filter(_ != '.').filter(_ != '?').filter(_ != ',').filter(_ != ';').filter(_ != '!').filter(_ != '-').filter(_ != ''')
    for (c <- s.filter(_ != '.').filter(_ != '?').filter(_ != ',').filter(_ != ';').filter(_ != '!').filter(_ != '-').filter(_ != ''')) {
      for (x <- s.filter(_ != '.').filter(_ != '?').filter(_ != ',').filter(_ != ';').filter(_ != '!').filter(_ != '-').filter(_ != ''').reverse) {
        if()
      }
    }
  }
